...
def score_single_prediction(pred_text: str, true_answer: str, max_optimal_length: int = 600) -> Dict[str, float]:
    ...
    - +0.1 points if the answer is correct AND the total output length is efficient (< max_optimal_length)
    - -0.2 penalty if tags are left unclosed (model rambled and got cut off).
    """
    ...
    # Penalize if it started a think tag but never closed it
